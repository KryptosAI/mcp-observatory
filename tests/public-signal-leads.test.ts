...
describe("public-signal leads", () => {
  ...
  it("surfaces one first scan and one CI next action while keeping the leads script unpublished", async () => {
    ...
    expect(site).toContain("Next: keep it running in CI");
    expect(site).toContain("setup-ci --all");
