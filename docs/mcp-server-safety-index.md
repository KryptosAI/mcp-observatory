...
Each row links to a reproducible command, a JSON run artifact, a Markdown report, and safe attack-simulation evidence generated by MCP Observatory. The goal is to show which failure classes matter before teams let agents depend on MCP servers. In short: MCP servers need [tool-call receipts](./tool-call-receipts.md), not badges.
