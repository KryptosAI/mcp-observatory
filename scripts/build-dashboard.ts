...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-width:320px;font-family:Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--text);background:radial-gradient(circle at 12% 0%,rgba(68,97,255,.2),transparent 32rem),radial-gradient(circle at 88% 12%,rgba(45,212,191,.12),transparent 28rem),var(--bg);line-height:1.55}
    ...
    @media(max-width:900px){.server-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.features{grid-template-columns:1fr}.feature-icon{margin-bottom:18px}.demo-section,.case-study{grid-template-columns:1fr}.social-proof{grid-template-columns:repeat(3,1fr)}.proof-stat{border-bottom:1px solid var(--line)}}@media(max-width:820px){.hero{grid-template-columns:1fr;padding-top:58px}.proof{max-width:520px}.navlinks a:not(.button){display:none}}@media(max-width:620px){.directory-tools{grid-template-columns:1fr}.server-grid{grid-template-columns:1fr}.server-card{min-height:230px}.social-proof{grid-template-columns:1fr 1fr}.try-command{align-items:flex-start;flex-direction:column}}@media(max-width:560px){.container{width:calc(100% - 28px)}h1{font-size:43px}.summary{grid-template-columns:1fr 1fr}.stat:last-child{grid-column:1/-1}th,td{padding:13px 12px}.subtitle{font-size:17px}.index-note{display:block}.index-note b{display:block;margin-bottom:4px}.case-study{padding:20px}}
  </style>
</head>
<body>
  <div class="container">
