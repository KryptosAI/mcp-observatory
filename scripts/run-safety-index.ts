...
import { setupCiHint } from "../src/commands/helpers.js";
...
function hasSchemaWarning(checks: CheckResult[]): boolean {
  return checks
    .filter((check) => check.id === "schema-quality")
    .some((check) => check.evidence.some((evidence) =>
      evidence.diagnostics?.some((diagnostic) => diagnostic.toLowerCase().startsWith("[warning]")) ||
      evidence.findings?.some((finding) => finding["severity"] === "warning"),
    ));
}
...
export function verdictForArtifact(artifact: RunArtifact): SafetyVerdict {
  ...
  const attackSim = artifact.checks.find((check) => check.id === "attack-sim");
  if (tools?.status === "fail") return "Not reproducible";
  if (attackSim?.status === "fail") return "Unsafe default posture";
  if (hasSecuritySeverity(artifact.checks, "high")) return "Unsafe default posture";
  if (attackSim?.status === "partial") return "Needs review before production";
  if (hasSchemaWarning(artifact.checks)) return "Needs review before production";
...
export function renderSafetyIndex(entries: SafetyIndexEntry[]): string {
  ...
  entries.forEach((entry, index) => {
    ...
    const receipt = buildActionReceipt(entry.artifact);
    const ciCommand = `${setupCiHint(targetConfigFor(entry.target))} --sarif --schedule weekly`;
    lines.push([
      `| ${index + 1}`,
