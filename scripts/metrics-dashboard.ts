...
function delta(current: number, previous: number, label = "vs prior period"): string {
...
function detailSearchRows(model: DashboardModel, points: UsageTrendPoint[]): string {
  ...
  add("KPI", "Market week", `${formatNumber(week.sessions)} sessions`, `${week.startDay} to ${week.endDay}; ${delta(week.sessions, previousWeek.sessions, "vs prior week")}`);
...
function featureStickinessPanel(model: DashboardModel): string {
  const commands = model.telemetry.commandFunnel || [];
  
  const totalSessions = model.telemetry.totalSessions || 1;
  ...
  const rows = commands
    ...
    .slice(0, 12)
    .map(c => {
      const share = totalSessions > 0 ? Math.round((c.sessions / totalSessions) * 100) : 0;
      const barWidth = Math.min(100, Math.max(2, share * 2));
      const stage = c.stage || 'unknown';
      
      const signals: string[] = [];
      if (stage.includes('CI') || stage.includes('Setup')) signals.push('ci');
      if (stage.includes('Paid')) signals.push('paid');
      if (stage.includes('Attack') || stage.includes('security')) signals.push('power');
      
      const stickinessScore = signals.length * 33 + (share > 10 ? 33 : share > 5 ? 15 : 0);
      const color = stickinessScore > 60 ? '#22c55e' : stickinessScore > 30 ? '#f59e0b' : '#64748b';
      
      return `<div class="command-row">
        <div class="command-head">
          <span class="cmd-name">${escapeHtml(stage)}</span>
          <span class="cmd-bar" style="width:${barWidth}%"></span>
          <span class="cmd-share">${share}%</span>
          <span class="cmd-arrow" style="color:${color}">${signals.includes('paid') ? '⚡' : signals.includes('ci') ? '🔧' : signals.includes('power') ? '🔒' : '—'} ${stickinessScore}pts</span>
        </div>
        <div class="command-sub">${formatNumber(c.sessions)} sessions${signals.length ? ' · ' + signals.join(' + ') : ''}</div>
      </div>`;
    })
    .join("");
  ...
  return `<section class="panel">
    <div class="panel-head">
      <h2>Feature Stickiness</h2>
      <span class="panel-note">Which features retain users · scores: ci=33pts, paid=33pts, power=33pts + share bonus</span>
    </div>
    <div class="command-list">
      ${rows}
    </div>
    <div style="padding:12px 16px;font-size:11px;color:var(--dim);">
      Stickiness scores upgrade with machineID tracking. Refresh telemetry for per-user retention data.
    </div>
  </section>`;
}

function userJourneyPanel(model: DashboardModel): string {
  const totalTelemetrySessions = model.telemetry.totalSessions || 1;
  const marketSessions = model.telemetry.marketSessions || 0;
  
  const stages = [
    { 
      name: 'Install', 
      count: totalTelemetrySessions,
      color: '#64748b',
      desc: 'Any telemetry event'
    },
    { 
      name: 'First Scan', 
      count: (model.telemetry.commandFunnel?.find(c => c.stage === 'Local validation')?.sessions || 0) +
             (model.telemetry.commandFunnel?.find(c => c.stage === 'Agent install')?.sessions || 0),
      color: '#3b82f6',
      desc: 'Ran test or scan command'
    },
    { 
      name: 'Recurring', 
      count: marketSessions,
      color: '#8b5cf6',
      desc: 'Multiple sessions'
    },
    { 
      name: 'CI Setup', 
      count: model.telemetry.commandFunnel?.find(c => c.stage === 'CI setup')?.sessions ?? 0,
      color: '#22c55e',
      desc: 'Integrated into CI pipeline'
    },
    { 
      name: 'Power User', 
      count: (model.telemetry.commandFunnel?.find(c => c.stage === 'Attack simulation')?.sessions ?? 0) +
             (model.telemetry.commandFunnel?.find(c => c.stage === 'Regression workflow')?.sessions ?? 0),
      color: '#f59e0b',
      desc: 'Advanced security features'
    },
    { 
      name: 'Paid Intent', 
      count: model.telemetry.commandFunnel?.find(c => c.stage === 'Paid intent')?.sessions ?? 0,
      color: '#ef4444',
      desc: 'Cloud, receipt, enterprise features'
    },
  ];
  ...
  const bars = stages.map((s, i) => {
    ...
    const nextCount = i < stages.length - 1 ? stages[i + 1]!.count : 0;
    const dropoff = s.count > 0 && nextCount > 0 
      ? Math.round(((s.count - nextCount) / s.count) * 100) 
      : 0;
    
    return `<div class="funnel-row" style="margin-bottom:8px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px">
        <span style="font-size:13px;color:${s.color};font-weight:600">${escapeHtml(s.name)}</span>
        <span style="font-size:12px;color:var(--muted)">${formatNumber(s.count)} · ${escapeHtml(s.desc)}</span>
      </div>
      <div style="height:6px;background:#1e293b;border-radius:3px;overflow:hidden">
        <div style="height:100%;width:${width}%;background:${s.color};border-radius:3px;transition:width .3s"></div>
      </div>
      ${dropoff > 0 ? `<div style="font-size:10px;color:var(--dim);text-align:right;margin-top:2px">${dropoff}% dropoff to next stage</div>` : ''}
    </div>`;
  }).join("");
  ...
  return `<section class="panel">
    <div class="panel-head">
      <h2>User Journey</h2>
      <span class="panel-note">Funnel from install to paid intent</span>
    </div>
    <div style="padding:8px 16px 16px">
      ${bars}
    </div>
    <div style="padding:0 16px 12px;font-size:11px;color:var(--dim);">
      MachineID tracking will provide exact per-user stage progression in future telemetry updates.
    </div>
  </section>`;
}

function renderPipelineRow(r: { domain: string; sessions: number; firstSeen: string; latestSeen: string; signals: Record<string, boolean>; readiness: number }): string {
  const accountType = classifyAccountType(r.domain);
  const typeClass = accountType.toLowerCase().replace(/\s+/g, '-');
  const readinessColor = r.readiness >= 60 ? '#22c55e' : r.readiness >= 30 ? '#f59e0b' : '#64748b';

  return `<div class="pipeline-row">
    <div class="pipeline-domain">
      <b>${escapeHtml(r.domain)}</b>
      <span class="type-badge type-${typeClass}">${escapeHtml(accountType)}</span>
    </div>
    <div class="pipeline-meta">
      <span>${formatNumber(r.sessions)} sessions</span>
      <span>First: ${formatShortDate(r.firstSeen)}</span>
      <span>Last: ${formatShortDate(r.latestSeen)}</span>
      <span class="pipeline-readiness" style="color:${readinessColor}">Readiness: ${r.readiness}/100</span>
    </div>
    <div class="pipeline-signals">
      ${Object.entries(r.signals).map(([name, active]) =>
        `<span class="signal-dot ${active ? 'signal-active' : ''}" title="${name}">${active ? '\u2713' : '\u2014'} ${name}</span>`
      ).join('')}
    </div>
  </div>`;
}

function pipelinePanel(model: DashboardModel): string {
  const domains = model.telemetry.topDomainDetails
    .filter(d => !INTERNAL_DOMAINS.has(d.domain) && !FREE_EMAIL_DOMAINS.has(d.domain))
    .slice(0, 15);

  const ciSetupStage = model.telemetry.commandFunnel.find(r => r.stage === 'CI setup');
  const riskGraphStage = model.telemetry.commandFunnel.find(r => r.stage === 'Risk graph');
  const receiptStage = model.telemetry.commandFunnel.find(r => r.stage === 'Receipts');
  const sarifStage = model.telemetry.commandFunnel.find(r => r.stage === 'SARIF export');
  const paidIntentStage = model.telemetry.commandFunnel.find(r => r.stage === 'Paid intent');

  const rows = domains.map(d => {
    const signals = {
      'setup-ci': (ciSetupStage?.sessions ?? 0) > 0,
      'risk-graph': (riskGraphStage?.sessions ?? 0) > 0,
      'receipt': (receiptStage?.sessions ?? 0) > 0,
      'sarif': (sarifStage?.sessions ?? 0) > 0,
      'paid-intent': (paidIntentStage?.sessions ?? 0) > 0,
    };
    const accountType = classifyAccountType(d.domain);
    const readiness = Object.values(signals).filter(Boolean).length * 20 +
      (accountType === 'Strategic' ? 20 : accountType === 'Enterprise' ? 10 : 0);
    return { domain: d.domain, sessions: d.sessions, firstSeen: d.firstSeen, latestSeen: d.latestSeen, signals, readiness };
  });
  rows.sort((a, b) => b.readiness - a.readiness);

  const high = rows.filter(r => r.readiness >= 60).length;
  const med = rows.filter(r => r.readiness >= 30 && r.readiness < 60).length;
  const low = rows.filter(r => r.readiness < 30).length;

  return `<section class="panel">
    <div class="panel-head">
      <h2>Pipeline</h2>
      <span class="panel-note">${rows.length} accounts &middot; ${high} high readiness &middot; ${med} medium &middot; ${low} low</span>
    </div>
    <div class="pipeline-table">
      ${rows.map(r => renderPipelineRow(r)).join('')}
    </div>
  </section>`;
}

function featureValuePanel(model: DashboardModel): string {
  const commands = model.telemetry.commandFunnel || [];
  const enterpriseDomains = model.telemetry.topDomainDetails.filter(
    d => !INTERNAL_DOMAINS.has(d.domain) && !FREE_EMAIL_DOMAINS.has(d.domain)
  );
  const enterpriseSessions = enterpriseDomains.reduce((sum, d) => sum + d.sessions, 0);
  const marketSessions = model.telemetry.marketSessions || 1;

  return `<section class="panel">
    <div class="panel-head">
      <h2>Feature Value</h2>
      <span class="panel-note">Which features attract high-value accounts</span>
    </div>
    <div class="simple-table">
      <div class="table-header">
        <span>Command</span><span>Sessions</span><span>Enterprise %</span><span>Conversion Signal</span>
      </div>
      ${commands.slice(0, 10).map(c => {
        const pct = c.sessions > 0 ? Math.round(((c.sessions / marketSessions) * enterpriseSessions / marketSessions) * 100) : 0;
        return `<div class="table-row">
          <span><code>${escapeHtml(c.stage || 'unknown')}</code></span>
          <span>${formatNumber(c.sessions)}</span>
          <span>${pct}%</span>
          <span>${pct > 30 ? '\u26A1 High intent' : pct > 15 ? '\uD83D\uDFF6 Medium' : '\u2014'}</span>
        </div>`;
      }).join('')}
    </div>
  </section>`;
...
function revenuePanel(model: DashboardModel): string {
  const pipelineCount = model.telemetry.topDomainDetails.filter(
    d => !INTERNAL_DOMAINS.has(d.domain) && !FREE_EMAIL_DOMAINS.has(d.domain)
  ).length;
  const paidIntentSessions = model.telemetry.commandFunnel?.find(r => r.stage === 'Paid intent')?.sessions ?? 0;

  return `<section class="panel">
    <div class="panel-head">
      <h2>Revenue</h2>
      <span class="panel-note">Ready \u2014 connect Stripe to populate</span>
    </div>
    <div class="signal-grid" style="grid-template-columns:repeat(4,1fr);padding:12px 16px">
      <div class="small-card"><span>MRR</span><strong>\u2014</strong><small>Connect Stripe</small></div>
      <div class="small-card"><span>Trials active</span><strong>\u2014</strong><small>Connect Stripe</small></div>
      <div class="small-card"><span>Pipeline accounts</span><strong>${formatNumber(pipelineCount)}</strong><small>Strategic + Enterprise</small></div>
      <div class="small-card"><span>Conversion signals</span><strong>${formatNumber(paidIntentSessions)}</strong><small>Paid intent sessions</small></div>
    </div>
  </section>`;
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .pipeline-table { padding: 8px 16px 16px; }
    .pipeline-row { padding: 12px 0; border-bottom: 1px solid var(--line); }
    .pipeline-row:last-child { border-bottom: none; }
    .pipeline-domain { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
    .pipeline-domain b { color: #e5edf7; font-size: 14px; }
    .pipeline-meta { display: flex; align-items: center; gap: 16px; font-size: 12px; color: var(--muted); margin-bottom: 6px; }
    .pipeline-readiness { font-weight: 600; }
    .pipeline-signals { display: flex; gap: 12px; flex-wrap: wrap; }
    .signal-dot { font-size: 11px; color: var(--dim); }
    .signal-dot.signal-active { color: #22c55e; font-weight: 600; }
    .simple-table { padding: 8px 16px 16px; }
    .simple-table .table-header, .simple-table .table-row { display: grid; grid-template-columns: 1.5fr 1fr 1fr 1fr; gap: 12px; padding: 8px 0; font-size: 13px; border-bottom: 1px solid var(--line); }
    .simple-table .table-header { color: var(--muted); font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; }
    .simple-table .table-row:last-child { border-bottom: none; }
    .simple-table .table-row code { background: transparent; color: var(--cyan); font-size: 12px; padding: 0; }
    .type-badge { font-size: 10px; padding: 2px 7px; border-radius: 4px; text-transform: uppercase; letter-spacing: 0.03em; }
    .type-strategic { background: rgba(239,68,68,.15); color: #ef4444; }
